    ...
    function ($) {
        ...
        searchLink.submit(function (event) {
            ...
            if (searchedMovie.trim().length == 0)
                movieSearchList.append(`<li>The input : ${JSON.stringify(searchedMovie)} needs to be movie. </li>`);
            else{

                    // console.log("Sup")
                    movieSearchList.append(`<p> Search Results for : ${JSON.stringify(searchedMovie)}</p>`);
                    ...
                    $.ajax(requestConfig).then(function (responseMessage) {
                        ...
                        $.each(allMovies, function(i, singleMovie){
                            ...
                            movieSearchList.append(`<li id="searchimage"> <a href = /moviedetails/${singleMovie._id}> <img src=${singleMovie.Movie_Poster}></a> ${singleMovie.Movie_Name}</li> <br>`);
                        })
                    });
